...
// Function prototypes
void arrSelectSort(int *[], int, bool reverseOrder = false); // Chú ? thêm tham s? reverseOrder
...
int main()
{
    ...
    // S?p x?p các ph?n t? c?a m?ng con tr? theo th? t? gi?m d?n.
    arrSelectSort(arrPtr, numDonations, true); // Ð?t tham s? reverseOrder thành true

    // Hi?n th? quyên góp b?ng cách s? d?ng m?ng con tr?. Ði?u này
    // s? hi?n th? chúng theo th? t? gi?m d?n.
    cout << "Các quyên góp, ðý?c s?p x?p theo th? t? gi?m d?n: \n";
...
void arrSelectSort(int *arr[], int size, bool reverseOrder)
{
    ...
    for (startScan = 0; startScan < (size - 1); startScan++)
    {
        ...
        {
            if (reverseOrder) {
                if (*(arr[index]) > *minElem) // So sánh ngý?c l?i n?u reverseOrder là true
                {
                    minElem = arr[index];
                    minIndex = index;
                }
